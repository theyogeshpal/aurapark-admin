...
<div class="login-wrapper d-flex align-items-center justify-content-center min-vh-100">
  <div class="login-card shadow-lg">
    <div class="text-center mb-4">
      ...
      <p class="text-muted small">Super Admin Panel Login</p>
    </div>
    ...
    <form (ngSubmit)="login()">
      <div class="mb-3">
        ...
        <div class="input-group">
          ...
          <input type="email" class="form-control border-start-0 bg-light" [(ngModel)]="form.email" name="email" placeholder="superadmin@aurapark.in" required>
        </div>
      </div>
      <div class="mb-4">
      ...
      <button type="submit" class="btn btn-dark w-100 py-2 fw-bold" [disabled]="loading()">
        <span *ngIf="!loading()">Login to Super Admin <i class="bi bi-arrow-right ms-1"></i></span>
        <span *ngIf="loading()"><i class="fa-solid fa-spinner fa-spin me-2"></i>Logging in...</span>
      </button>
      <div class="text-center mt-3">
        <a routerLink="/admin/login" class="small text-muted">Admin? Login here</a>
      </div>
    </form>
  </div>
</div>
  `,
  styles: [`
    .login-wrapper { background: linear-gradient(135deg, #1a1c23 0%, #2d3561 100%); }
...
export class SaLoginComponent {
  ...
  login() {
    ...
    this.auth.saLogin(this.form.email, this.form.password).subscribe({
